...
void setup() {
    ...
    digitalWrite(A8, LOW);
    setNose(NOSE_CENTER + 20);//Drop the nose by turning a little bit left
    delay(200); //wait for the nose to fall down
    ...
    for(int i = 0; i < 160; ++i) {
      if(i == 95) {
        setNose(NOSE_CENTER - 5);
          driveWheels(255, 200); //Starts driving  
      }
      ringServo.write(i); //swings the ring arm
      delay(3); //don't swing the arm too fast
    }
    
    
    setNose(NOSE_CENTER - 40);
    //updateSensors();
    
    //********Is this necessary??
    delay(210); //go blindly straight for half second before starting to steer
