...
void rings::dropRings() {
  ...
  if (!isDropping) {
    ...
    (stateCounter == 5 || stateCounter == 6 || stateCounter == 12 || stateCounter == 13 || stateCounter == 18 || stateCounter == 19
    || stateCounter == 25 || stateCounter == 26 || stateCounter == 30 || stateCounter == 31 )){
      dropCounter++;
      if (stateCounter == 5|| stateCounter ==6){
        dropRing();
      }else{
        initDrop(true);
      }
...
void rings::initDrop(bool newInit) {
  static long timeOfInit;
  static bool initCalled = false;
  
  if (!initCalled && newInit) {
    initCalled = true;
    timeOfInit = millis();
  }
  
  if (initCalled && ((millis() - timeOfInit) > RING_DROP_DELAY) ) {
    initCalled = false;
    dropRing();
  }
